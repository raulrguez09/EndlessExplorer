...
void main() {
    ...
    // 1postion our geometry - coordinates your object begins in.
    vec4 localPosition = vec4(newPos, 1.0);

    // 2transform the local coordinates to world-space coordinates
    vec4 worldPosition = modelMatrix * localPosition;
    
    // 3transform the world coordinates to view-space coordinates - seen from the camera/viewer point of view
    vec4 viewPosition = viewMatrix * worldPosition;

    // 4project view coordinates to clip coordinates and transform to screen coordinates
